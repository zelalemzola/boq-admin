...
const Partners = () => {
  ...
  return (
    <div>
      <div className="fixed bg-gray-50 shadow-xl z-30 w-full px-4 py-3 border-b  flex flex-col lg:flex-row items-center gap-6 lg:gap-20">
        <Drawer>
          <DrawerTrigger>
  ...
  <table className="min-w-full bg-white w-[90%] md:w-full border">
    <thead className="bg-gray-800 text-white">
      <tr className='border'>
        ...
        <th className="py-3 px-4 uppercase font-semibold text-sm text-center">Photo</th>
    ...
    <tbody className='text-gray-700 '>
      {filteredPartners.map((partner) => (
        <tr key={partner._id} className='border-b '>
          {/* Added text-center to center the body content */}
          <td className="py-3 px-4 text-center">{partner.name}</td>
          <td className="py-3 px-4 text-center">
            {/* Centering the image with mx-auto for horizontal centering */}
            <img src={partner.photoUrl} alt={partner.name} width={50} height={50} className="mx-auto" />
          </td>
          <td className="py-3 px-4 text-center">
            {/* Flexbox to align the buttons within their cell */}
